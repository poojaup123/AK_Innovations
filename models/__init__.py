...
# Note: GRN models imported separately to avoid circular imports
...
class ComponentJobCard(db.Model):
    """Component-Level Job Cards - Individual job cards for each BOM component"""
    __tablename__ = 'component_job_cards'
    
    id = db.Column(db.Integer, primary_key=True)
    job_card_number = db.Column(db.String(50), unique=True, nullable=False)
    
    # Production Order Connection
    production_id = db.Column(db.Integer, db.ForeignKey('productions.id'), nullable=False)
    bom_id = db.Column(db.Integer, db.ForeignKey('boms.id'), nullable=False)
    bom_item_id = db.Column(db.Integer, db.ForeignKey('bom_items.id'), nullable=False)
    
    # Component Details
    component_id = db.Column(db.Integer, db.ForeignKey('items.id'), nullable=False)  # The material/component
    component_name = db.Column(db.String(200), nullable=False)  # Cache component name for quick access
    
    # Quantities - Planned vs Actual
    planned_quantity = db.Column(db.Float, nullable=False)
    actual_quantity_consumed = db.Column(db.Float, default=0.0)
    remaining_quantity = db.Column(db.Float, default=0.0)
    scrap_quantity = db.Column(db.Float, default=0.0)
    
    # Units of Measure
    planned_uom = db.Column(db.String(20), default='pcs')
    actual_uom = db.Column(db.String(20), default='pcs')
    
    # Worker/Vendor Assignment
    assigned_worker_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # Assigned worker
    assigned_department = db.Column(db.String(100), nullable=True)  # Department handling this component
    assigned_vendor_id = db.Column(db.Integer, db.ForeignKey('suppliers.id'), nullable=True)  # Outsourced vendor
    work_type = db.Column(db.String(20), default='in_house')  # in_house, outsourced
    ...
    # Status Tracking
    status = db.Column(db.String(20), default='planned')  # planned, issued, in_progress, completed, on_hold
    progress_percentage = db.Column(db.Float, default=0.0)
    
    # Process Steps (inherited from BOM routing)
    process_steps = db.Column(db.Text)  # JSON array of process steps from BOMProcess
    current_process_step = db.Column(db.Integer, default=1)  # Current step number
    ...
    # Cost Tracking
    estimated_cost = db.Column(db.Float, default=0.0)  # Estimated cost from BOM
    actual_cost = db.Column(db.Float, default=0.0)  # Actual cost incurred
    labor_cost = db.Column(db.Float, default=0.0)  # Labor cost for this component
    
    # Batch Tracking
    input_batch_ids = db.Column(db.Text)  # JSON array of input batch IDs consumed
    output_batch_id = db.Column(db.Integer, db.ForeignKey('item_batches.id'), nullable=True)  # Output batch created
    
    # Issue Tracking
    delay_reason = db.Column(db.String(200), nullable=True)  # Reason for delay if any
    on_hold_reason = db.Column(db.String(200), nullable=True)  # Reason for hold if any
    issues_encountered = db.Column(db.Text)  # JSON array of issues
    resolution_notes = db.Column(db.Text)  # Resolution details
    
    # Priority & Dependencies
    priority = db.Column(db.String(20), default='normal')  # low, normal, high, urgent
    dependencies = db.Column(db.Text)  # JSON array of dependent job card IDs
    blocking_cards = db.Column(db.Text)  # JSON array of job cards blocked by this one
    ...
    # Relationships
    production = db.relationship('Production', backref='component_job_cards')
    bom = db.relationship('BOM', backref='component_job_cards')
    bom_item = db.relationship('BOMItem', backref='component_job_cards')
    component = db.relationship('Item', foreign_keys=[component_id], backref='component_job_cards')
    assigned_worker = db.relationship('User', foreign_keys=[assigned_worker_id], backref='assigned_job_cards')
    assigned_vendor = db.relationship('Supplier', foreign_keys=[assigned_vendor_id], backref='assigned_job_cards')
    inspector = db.relationship('User', foreign_keys=[inspected_by_id], backref='inspected_job_cards')
    output_batch = db.relationship('ItemBatch', foreign_keys=[output_batch_id], backref='job_card_output')
    creator = db.relationship('User', foreign_keys=[created_by_id], backref='created_job_cards')
    
    @classmethod
    def generate_job_card_number(cls):
        """Generate unique job card number"""
        current_year = datetime.now().year
        # Find last job card number for current year
        last_card = cls.query.filter(cls.job_card_number.like(f'JC-{current_year}-%')).order_by(cls.id.desc()).first()
        if last_card:
            try:
                last_sequence = int(last_card.job_card_number.split('-')[-1])
                next_sequence = last_sequence + 1
            except (ValueError, IndexError):
                next_sequence = 1
        else:
            next_sequence = 1
        return f"JC-{current_year}-{next_sequence:04d}"
    
    @classmethod
    def create_job_cards_from_production(cls, production_id, created_by_id):
        """Create job cards for all BOM components in a production order"""
        ...
        try:
            for bom_item in bom.items:
                # Create job card for each BOM component
                job_card = cls(
                    job_card_number=cls.generate_job_card_number(),
                    production_id=production_id,
                    bom_id=bom.id,
                    bom_item_id=bom_item.id,
                    component_id=bom_item.material_id,
                    component_name=bom_item.material.name if bom_item.material else 'Unknown Component',
                    planned_quantity=bom_item.qty_required * production.quantity_planned,
                    remaining_quantity=bom_item.qty_required * production.quantity_planned,
                    planned_uom=bom_item.unit,
                    actual_uom=bom_item.unit,
                    target_completion_date=production.production_date,
                    estimated_cost=bom_item.total_cost,
                    created_by_id=created_by_id,
                    # Load process steps from BOM routing
                    process_steps=cls._get_process_steps_json(bom),
                    inspection_required=bom_item.is_critical
                )
                
                db.session.add(job_card)
                job_cards_created.append(job_card)
            
            db.session.commit()
            return True, f"Created {len(job_cards_created)} job cards successfully"
    ...
    @staticmethod
    def _get_process_steps_json(bom):
        """Convert BOM processes to JSON for job card"""
        import json
        processes = []
    ...
    @property
    def completion_percentage(self):
        """Calculate completion percentage based on quantities"""
        if self.planned_quantity > 0:
            consumed_percentage = (self.actual_quantity_consumed / self.planned_quantity) * 100
            return min(consumed_percentage, 100)
        return 0.0
    
    @property
    def is_delayed(self):
        """Check if job card is delayed"""
        if self.target_completion_date and self.status not in ['completed']:
    ...
    def days_remaining(self):
        ...
        if self.target_completion_date:
            ...
            delta = self.target_completion_date - today
            return delta.days
        return None
    
    def update_progress(self, quantity_consumed, process_step=None, notes=None):
        """Update job card progress"""
        try:
            self.actual_quantity_consumed += quantity_consumed
            self.remaining_quantity = max(0, self.planned_quantity - self.actual_quantity_consumed)
            ...
            if notes:
                if self.resolution_notes:
                    ...
                    self.resolution_notes = f"{datetime.utcnow().strftime('%Y-%m-%d %H:%M')}: {notes}"
            ...
            elif self.actual_quantity_consumed > 0:
                self.status = 'in_progress'
    ...
    def __repr__(self):
        return f'<ComponentJobCard {self.job_card_number}: {self.component_name}>'

class ComponentJobCardProcess(db.Model):
    """Track individual process steps within component job cards"""
    __tablename__ = 'component_job_card_processes'
    
    id = db.Column(db.Integer, primary_key=True)
    job_card_id = db.Column(db.Integer, db.ForeignKey('component_job_cards.id'), nullable=False)
    process_step = db.Column(db.Integer, nullable=False)
    process_name = db.Column(db.String(100), nullable=False)
    process_code = db.Column(db.String(20))
    
    # Status and timing
    status = db.Column(db.String(20), default='planned')  # planned, in_progress, completed, skipped
    started_at = db.Column(db.DateTime)
    completed_at = db.Column(db.DateTime)
    estimated_duration_minutes = db.Column(db.Float, default=0.0)
    actual_duration_minutes = db.Column(db.Float, default=0.0)
    
    # Worker assignment for this specific process
    assigned_worker_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    department_id = db.Column(db.Integer, db.ForeignKey('departments.id'))
    
    # Process-specific quantities
    input_quantity = db.Column(db.Float, default=0.0)
    output_quantity = db.Column(db.Float, default=0.0)
    scrap_quantity = db.Column(db.Float, default=0.0)
    
    # Quality and notes
    quality_status = db.Column(db.String(20), default='pending')
    ...
    # Relationships
    job_card = db.relationship('ComponentJobCard', backref='job_card_processes')
    assigned_worker = db.relationship('User', backref='assigned_process_steps')
    department = db.relationship('Department', backref='job_card_processes')
    
    @property
    def status_badge_class(self):
        """Return Bootstrap badge class for process status"""
        status_classes = {
            'planned': 'bg-light text-dark',
            'in_progress': 'bg-warning',
            'completed': 'bg-success',
