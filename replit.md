...
- **Enhanced BOM Management with Partial Production:** Advanced Bill of Materials system with enhanced features including partial production capability, component type classification (raw materials, sub-assemblies, phantom BOMs, packing materials, consumables), and intelligent cost calculations. Features proper labor cost inheritance for nested BOMs ensuring per-unit costs are correctly calculated without double division. Supports flexible production quantities instead of fixed batch sizes, allowing incomplete production runs with proportional material calculations. Includes enhanced BOM process management with lead time tracking, vendor assignment, skill requirements, and priority levels. The system provides comprehensive cost breakdowns with real-time calculation updates and validation for partial production feasibility.
