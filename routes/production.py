...
@production_bp.route('/enhanced-bom', methods=['GET', 'POST'])
@production_bp.route('/enhanced-bom/<int:bom_id>', methods=['GET', 'POST'])
@login_required
def enhanced_bom(bom_id=None):
    """Enhanced BOM creation and editing with partial production features"""
    from forms_bom_enhanced import EnhancedBOMForm
    
    form = EnhancedBOMForm()
    bom = None
    
    if bom_id:
        bom = BOM.query.get_or_404(bom_id)
        # Pre-populate form with existing BOM data
        if request.method == 'GET':
            form.bom_code.data = bom.bom_code
            form.product_id.data = bom.product_id
            form.version.data = bom.version
            ...
            form.unit_weight.data = bom.unit_weight
            form.unit_weight_uom.data = bom.unit_weight_uom
            form.allow_partial_production.data = bom.allow_partial_production
            form.allow_substitute_items.data = bom.allow_substitute_items
            form.auto_create_job_work.data = bom.auto_create_job_work
            form.auto_batch_assignment.data = bom.auto_batch_assignment
            form.is_phantom_bom.data = bom.is_phantom_bom
            form.intermediate_product.data = bom.intermediate_product
            form.labor_cost_per_unit.data = bom.labor_cost_per_unit
            form.overhead_cost_per_unit.data = bom.overhead_cost_per_unit
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            if not bom:
                bom = BOM()
                # Generate unique BOM code if not provided
                if not form.bom_code.data:
                    bom.bom_code = f"BOM-{datetime.now().strftime('%Y%m%d')}-{BOM.query.count() + 1:04d}"
                else:
                    bom.bom_code = form.bom_code.data
            
            # Update BOM fields
            bom.product_id = form.product_id.data
            bom.version = form.version.data or '1.0'
            bom.description = form.description.data
            bom.output_quantity = form.output_quantity.data
            bom.output_uom_id = form.output_uom_id.data if form.output_uom_id.data != 0 else None
            bom.unit_weight = form.unit_weight.data
            bom.unit_weight_uom = form.unit_weight_uom.data
            
            # Enhanced features
            bom.allow_partial_production = form.allow_partial_production.data
            bom.allow_substitute_items = form.allow_substitute_items.data
            bom.auto_create_job_work = form.auto_create_job_work.data
            bom.auto_batch_assignment = form.auto_batch_assignment.data
            bom.is_phantom_bom = form.is_phantom_bom.data
            bom.intermediate_product = form.intermediate_product.data
            
            # Cost settings
            bom.labor_cost_per_unit = form.labor_cost_per_unit.data
            bom.overhead_cost_per_unit = form.overhead_cost_per_unit.data
            bom.overhead_percentage = form.overhead_percentage.data
            bom.freight_cost_per_unit = form.freight_cost_per_unit.data
            bom.freight_unit_type = form.freight_unit_type.data
            bom.markup_percentage = form.markup_percentage.data
            
            # Scrap settings
            bom.estimated_scrap_percent = form.estimated_scrap_percent.data
            bom.scrap_quantity = form.scrap_quantity.data
            bom.scrap_value_recovery_percent = form.scrap_value_recovery_percent.data
            bom.remarks = form.remarks.data
            
            # Set status based on submit button
            if 'save_active' in request.form:
                bom.is_active = True
                bom.status = 'active'
            else:
                bom.is_active = False
            ...
            # Set creation info for new BOM
            if not bom_id:
                bom.created_by = current_user.id
                bom.created_at = datetime.utcnow()
            ...
            flash_message = 'Enhanced BOM updated successfully' if bom_id else 'Enhanced BOM created successfully'
            if bom.allow_partial_production:
                flash_message += ' with partial production capability'
            
            flash(flash_message, 'success')
            return redirect(url_for('production.enhanced_bom', bom_id=bom.id))
            
        except Exception as e:
            db.session.rollback()
            flash(f'Error saving enhanced BOM: {str(e)}', 'error')
    
    # Get materials data for existing BOM
    materials_data = []
        ...
        for item in bom.items:
            ...
            if material:
                materials_data.append({
                    'id': item.id,
                    'material': material,
                    'qty_required': item.qty_required or item.quantity_required,
                    'unit_cost': item.unit_cost,
                    'component_type': getattr(item, 'component_type', 'raw_material'),
                    'is_critical': getattr(item, 'is_critical', False),
                    'is_phantom': getattr(item, 'is_phantom', False),
                    'scrap_percent': item.scrap_percent,
                    'total_cost': (item.qty_required or item.quantity_required or 0) * item.unit_cost
                })
    
    return render_template('production/enhanced_bom_form.html', 
                         form=form, 
                         bom=bom, 
                         materials_data=materials_data,
                         title='Enhanced BOM' + (f' - {bom.bom_code}' if bom else ''))

@production_bp.route('/api/partial-production-preview', methods=['POST'])
@login_required
def api_partial_production_preview():
    """API endpoint for partial production requirements preview"""
    try:
        data = request.get_json()
        bom_id = data.get('bom_id')
        target_quantity = float(data.get('target_quantity', 0))
        
        if not bom_id or target_quantity <= 0:
            return jsonify({'error': 'Invalid BOM ID or target quantity'}), 400
        
        bom = BOM.query.get_or_404(bom_id)
        
        # Validate partial production
        is_valid, message = bom.validate_partial_production(target_quantity)
        if not is_valid:
            return jsonify({'error': message}), 400
        
        # Calculate requirements
        requirements = bom.calculate_partial_production_requirements(target_quantity)
        
        # Format response
        response = {
            'valid': True,
            'requirements': {
                'target_quantity': requirements['target_quantity'],
                'scaling_factor': requirements['scaling_factor'],
                'total_cost': requirements['total_cost'],
                'labor_cost': requirements['labor_cost'],
                'materials': []
            }
        }
        ...
        for material_req in requirements['materials']:
            ...
            response['requirements']['materials'].append({
                'id': material.id,
                'name': material.name,
                'code': material.code,
                'standard_qty': material_req['standard_qty'],
                'partial_qty': material_req['partial_qty'],
                'unit': material_req['unit'],
                'unit_cost': material_req['unit_cost'],
                'total_cost': material_req['total_cost'],
                'component_type': material_req['component_type'],
                'is_critical': material_req['is_critical']
            })
        
        return jsonify(response)
        
    except Exception as e:
        return jsonify({'error': f'Error calculating partial production: {str(e)}'}), 500

@production_bp.route('/partial-production', methods=['GET', 'POST'])
@login_required
def partial_production():
    """Partial production planning form"""
    from forms_bom_enhanced import PartialProductionForm
    
    form = PartialProductionForm()
    
    if request.method == 'POST' and form.validate_on_submit():
        try:
            bom = BOM.query.get_or_404(form.bom_id.data)
            
            # Validate partial production
            is_valid, message = bom.validate_partial_production(form.target_quantity.data)
            if not is_valid:
                flash(f'Partial production validation failed: {message}', 'error')
                return render_template('production/partial_production_form.html', form=form, today_date=date.today().isoformat())
            
            # Calculate requirements
            requirements = bom.calculate_partial_production_requirements(form.target_quantity.data)
            
            # Create a production order for partial production
            from utils import generate_production_number
            production = Production(
                production_number=generate_production_number(),
                bom_id=bom.id,
                quantity_to_produce=form.target_quantity.data,
                production_date=datetime.strptime(form.production_date.data, '%Y-%m-%d').date(),
                status='planned',
                priority=int(form.priority.data),
                notes=f"Partial Production: {form.reason.data}\n\nAdditional Notes: {form.notes.data}",
                created_by=current_user.id,
                created_at=datetime.utcnow()
            )
            
            # Set partial production flags
            production.is_partial_production = True
            production.scaling_factor = requirements['scaling_factor']
            production.ignore_material_shortage = form.ignore_material_shortage.data
            production.use_substitute_materials = form.use_substitute_materials.data
            
            db.session.add(production)
            db.session.commit()
            
            flash(f'Partial production planned successfully! Production Number: {production.production_number}', 'success')
            return redirect(url_for('production.production_detail', id=production.id))
            
        except Exception as e:
            db.session.rollback()
            flash(f'Error planning partial production: {str(e)}', 'error')
    
    return render_template('production/partial_production_form.html', 
                         form=form, 
                         today_date=date.today().isoformat(),
                         title='Partial Production Planning')
